...
function decodeValueFromHex(layerId: string, hex: string): string | null {
  ...
  if (layerId === 'maxz' || layerId === 'cappi2km')
...
function getLayerInstanceById(id: string, target: 'main' | 'secondary'): ImageSequenceLayer | null {
  if (target === 'main') {
    ...
    if (id === 'cappi2km') return layers.cappi2km
    ...
    if (id === 'cappi2km') return layers.cappi2kmSecondary
...
onMounted(async () => {
  ...
  await Promise.all([
    ...
    initLayer(layers.cappi2km, mapObject, oneWeekAgoTimestamp.value, currentTimestamp.value, true),
...
watch(
  ...
  ([enabled, ts]) => {
    ...
    rafMain = requestAnimationFrame(() => {
      if (!auth.hasFullLinkAccess) return
      ...
      <div class="relative flex h-full w-full flex-row items-center justify-center">
        ...
        <TopNavbar v-show="!config.hideUI">
          <div class="mr-32 hidden gap-x-2 md:flex">
            ...
            <button v-if="!config.realtime && auth.hasFullLinkAccess" id="user-calc-button"
          ...
          <template #settings>
            ...
            <div v-if="links.hasLinks && auth.hasFullLinkAccess" class="flex w-full flex-col gap-y-1 pb-2">
              <div class="flex items-center gap-x-2">
        ...
        <LeftMenu v-show="!config.hideUI">
          ...
          <template #down>
            <Icon v-if="selectedLinkIds.size !== 0 && auth.hasFullLinkAccess" icon="clarity:copy-to-clipboard-line" width="38"
              height="38" class="menu-btn" @click="copySelectedLinksToClipboard" />
          </template>
        </LeftMenu>
        ...
        <ReflectivityBar v-if="
          (activeLayerSecondary?.id == 'maxz' ||
            activeLayerSecondary?.id == 'cappi2km' ||
